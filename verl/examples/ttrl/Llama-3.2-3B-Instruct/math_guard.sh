...
DATA_LOCAL_DIR="path/to/TTRL-Guard/verl/data"
BACKBONE_PATH="path/to/${BACKBONE}"

MODEL="${TASK}-${BACKBONE}"
EXPERIMENT="TTRL-Len@${K}k-${METHOD}"
...
python3.12 -m verl.trainer.main_ppo \
  ...
  # TTRL-Guard optimal config: MPS+RCSU (FRS disabled: hurts AIME24 20.3->17.1, RCSU wins AMC, MPS wins AIME24)
  +guard.enable=True \
  +guard.enable_frs=False \
  +guard.enable_mps=True \
  +guard.enable_rcsu=True \
  +guard.tau_fr=0.3 \
  +guard.tau_mr=0.6 \
  +guard.lambda1=0.5 \
  +guard.lambda2=0.3 \
  +guard.beta_max=0.3 \
  +guard.mps_min_votes_denom=4 \
  +guard.mps_steady=3 \
  +guard.rcsu_window=5 \
  +guard.rcsu_theta_mr=0.5 \
